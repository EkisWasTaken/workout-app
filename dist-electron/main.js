...
const STRAVA_CLIENT_ID = "191118";
const STRAVA_CLIENT_SECRET = "b9e9c9a8f821a88aa1e48ac780a71422921871a4";
...
async function exchangeCodeForToken(code) {
  ...
  try {
    ...
    console.error("Strava: Error exchanging code for token:", ((_a = error.response) == null ? void 0 : _a.data) || error.message);
    throw error;
  }
}
async function getActivities(accessToken, page = 1, per_page = 30) {
  var _a;
  try {
    ...
    console.error("Strava: Error fetching athlete activities:", ((_a = error.response) == null ? void 0 : _a.data) || error.message);
    throw error;
  }
}
async function getStravaActivityById(accessToken, activityId) {
  try {
...
let win = null;
const dbPath = path$1.join(app.getPath("userData"), "workouts.db");
  ...
  if (err) {
    ...
    db.serialize(() => {
      db.run(`CREATE TABLE IF NOT EXISTS workouts (
        ...
        if (err2) {
          console.error("Error creating workouts table:", err2);
        } else {
          console.log("Workouts table checked/created successfully.");
          db.all("PRAGMA table_info(workouts)", (err3, columns) => {
            if (err3) {
              console.error("Error getting table info:", err3);
              return;
            }
            if (Array.isArray(columns)) {
              const hasStravaId = columns.some((col) => col.name === "stravaActivityId");
              if (!hasStravaId) {
                db.run("ALTER TABLE workouts ADD COLUMN stravaActivityId TEXT", (err4) => {
                  if (err4) console.error("Error adding stravaActivityId column:", err4);
                  else console.log("stravaActivityId column added successfully.");
                });
              }
              const hasIsDeleted = columns.some((col) => col.name === "isDeleted");
              if (!hasIsDeleted) {
                db.run("ALTER TABLE workouts ADD COLUMN isDeleted INTEGER DEFAULT 0", (err4) => {
                  if (err4) console.error("Error adding isDeleted column:", err4);
              ...
              if (!hasActualDuration) {
                db.run("ALTER TABLE workouts ADD COLUMN actualDuration INTEGER", (err4) => {
                  if (err4) console.error("Error adding actualDuration column:", err4);
                  else console.log("actualDuration column added successfully.");
                });
              }
              const hasDuration = columns.some((col) => col.name === "duration");
              if (!hasDuration) {
                db.run("ALTER TABLE workouts ADD COLUMN duration INTEGER", (err4) => {
                  if (err4) console.error("Error adding duration column:", err4);
              ...
              if (!hasCaloriesBurned) {
                db.run("ALTER TABLE workouts ADD COLUMN caloriesBurned INTEGER", (err4) => {
                  if (err4) console.error("Error adding caloriesBurned column:", err4);
                  else console.log("caloriesBurned column added successfully.");
                });
              }
              const hasRpe = columns.some((col) => col.name === "rpe");
              if (!hasRpe) {
                db.run("ALTER TABLE workouts ADD COLUMN rpe INTEGER", (err4) => {
                  if (err4) console.error("Error adding rpe column:", err4);
                  else console.log("rpe column added successfully.");
                });
              }
              const hasNotes = columns.some((col) => col.name === "notes");
              if (!hasNotes) {
                db.run("ALTER TABLE workouts ADD COLUMN notes TEXT", (err4) => {
                  if (err4) console.error("Error adding notes column:", err4);
              ...
              if (!hasTotalWeightLifted) {
                db.run("ALTER TABLE workouts ADD COLUMN totalWeightLifted INTEGER", (err4) => {
                  if (err4) console.error("Error adding totalWeightLifted column:", err4);
                  else console.log("totalWeightLifted column added successfully.");
                });
              }
              const hasDistance = columns.some((col) => col.name === "distance");
              if (!hasDistance) {
                db.run("ALTER TABLE workouts ADD COLUMN distance REAL", (err4) => {
                  if (err4) console.error("Error adding distance column:", err4);
          ...
          athlete_id TEXT PRIMARY KEY
            ...
            if (err3) {
              console.error("Error creating strava_tokens table:", err3);
            } else {
          ...
          color TEXT NOT NULL
            ...
            if (err3) {
              console.error("Error creating workout_type_colors table:", err3);
            } else {
              console.log("Workout_type_colors table checked/created successfully.");
              db.get("SELECT COUNT(*) as count FROM workout_type_colors", (err4, row) => {
                if (err4) {
                  console.error("Error checking workout_type_colors count:", err4);
                  return;
                }
                if (row.count === 0) {
          ...
          notes TEXT
            ...
            if (err3) {
              console.error("Error creating nutritions table:", err3);
            } else {
          ...
          weight REAL NOT NULL
            ...
            if (err3) {
              console.error("Error creating daily_weights table:", err3);
            } else {
          ...
          name TEXT NOT NULL UNIQUE
            ...
            if (err3) {
              console.error("Error creating workout_templates table:", err3);
            } else {
          ...
          FOREIGN KEY (template_id) REFERENCES workout_templates(id)
            ...
            if (err3) {
              console.error("Error creating workout_template_exercises table:", err3);
            } else {
...
async function getValidStravaAccessToken() {
  ...
  try {
    ...
    const expiresAt = Math.floor(Date.now() / 1e3) + refreshedTokens.expires_in;
    const newTokens = {
...
function createStravaAuthWindow() {
  ...
  const handleNavigation = (_event, url2) => {
    console.log("Auth window navigation:", url2);
    const stravaRedirectUri = "http://localhost:5173/strava-callback";
    if (url2.startsWith(stravaRedirectUri)) {
      console.log("Strava redirect intercepted:", url2);
      if (win) {
        win.focus();
        win.webContents.send("strava-auth-callback", url2);
      }
      authWindow.close();
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle("strava-exchange-code", async (_event, code) => {
    try {
      ...
      const tokensToSave = {
        ...
        expires_at: expiresAt,
        athlete_id: tokenResponse.athlete.id.toString()
  ...
  const ACTIVITIES_CACHE_TTL = 15 * 60 * 1e3;
  const detailedActivityCache = /* @__PURE__ */ new Map();
  const DETAILED_CACHE_TTL = 30 * 60 * 1e3;
  let lastRequestTime = 0;
  const MIN_REQUEST_INTERVAL = 2e3;
  ...
  async function getPersistentCache(id) {
    return new Promise((resolve) => {
      db.get("SELECT data, timestamp FROM strava_cache WHERE id = ?", [id], (err, row) => {
        if (err || !row) return resolve(null);
        const age = Date.now() - row.timestamp;
        const ttl = id.startsWith("list_") ? ACTIVITIES_CACHE_TTL : 7 * 24 * 60 * 60 * 1e3;
        if (age < ttl) {
          try {
  ...
  async function setPersistentCache(id, type2, data) {
    const dataStr = JSON.stringify(data);
    const timestamp = Date.now();
    db.run("INSERT OR REPLACE INTO strava_cache (id, type, data, timestamp) VALUES (?, ?, ?, ?)", [id, type2, dataStr, timestamp]);
  }
  ipcMain.handle("strava-get-activities", async (_event, page, per_page) => {
    var _a;
    console.log("Main: strava-get-activities called", { page, per_page });
    const isDefaultRequest = (page === void 0 || page === 1) && (per_page === void 0 || per_page === 30);
    ...
    try {
      ...
      return { error: "API_ERROR", details: ((_a = error.response) == null ? void 0 : _a.data) || error.message };
    }
  });
  ipcMain.handle("get-strava-activity-by-id", async (_event, activityId) => {
    var _a;
    ...
    if (cachedActivities && Array.isArray(cachedActivities)) {
      const summary = cachedActivities.find((a) => a.id.toString() === activityId);
  ...
  ipcMain.handle("add-workout", async (_event, workout) => {
    return new Promise((resolve, reject) => {
      db.run(
  ...
  ipcMain.handle("update-workout", async (_event, workout) => {
    return new Promise((resolve, reject) => {
      db.run(
  ...
  ipcMain.handle("delete-workout", async (_event, id) => {
  ...
  ipcMain.handle("delete-workouts", async (_event, ids) => {
  ...
  ipcMain.handle("get-completed-workouts", async (_event, limit, offset) => {
    return new Promise((resolve, reject) => {
  ...
  ipcMain.handle("complete-workout", async (_event, data) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (actualDuration !== void 0) {
        columns.push("actualDuration = ?");
      ...
      if (totalWeightLifted !== void 0) {
        columns.push("totalWeightLifted = ?");
        values.push(totalWeightLifted);
      }
      if (stravaActivityId !== void 0) {
        columns.push("stravaActivityId = ?");
        values.push(stravaActivityId);
      }
  ...
  ipcMain.handle("read-file", async (_event, filePath) => {
    try {
      const content = await fs$1.readFile(filePath, { encoding: "utf-8" });
  ...
  ipcMain.handle("get-workout-by-id", async (_event, id) => {
    return new Promise((resolve, reject) => {
  ...
  ipcMain.handle("set-workout-type-color", async (_event, { type: type2, color }) => {
    return new Promise((resolve, reject) => {
      db.run("INSERT OR REPLACE INTO workout_type_colors (type, color) VALUES (?, ?)", [type2, color], function(err) {
        if (err) {
          reject(err);
        } else {
          resolve(this.changes);
  ...
  ipcMain.handle("add-nutrition", async (_event, nutrition) => {
    return new Promise((resolve, reject) => {
      db.run(
  ...
  ipcMain.handle("add-daily-weight", async (_event, dailyWeight) => {
    return new Promise((resolve, reject) => {
      db.run(
  ...
  ipcMain.handle("get-workout-template-exercises", async (_event, templateId) => {
    return new Promise((resolve, reject) => {
  ...
  ipcMain.handle("add-workout-template", async (_event, { name, exercises }) => {
    return new Promise((resolve, reject) => {
      db.run("INSERT INTO workout_templates (name) VALUES (?)", [name], function(err) {
        ...
        Promise.all(exercisesPromises).then(() => resolve(templateId)).catch(reject);
      });
    });
  });
  ipcMain.handle("delete-workout-template", async (_event, templateId) => {
    return new Promise((resolve, reject) => {
      db.serialize(() => {
